...
- Added CZSC-BI derived line segments for the Kline structure layer when CZSC does not expose native segment objects, preserving `source=derived_from_czsc_bis_v1` so the UI can display line segments without restoring legacy `chan.py`.
...
- Playwright smoke for Kline CZSC derived segments: existing snapshots show `49 笔 · 10 线段` and render segment SVG without failed requests.
