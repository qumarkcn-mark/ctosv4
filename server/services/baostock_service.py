...
# 冷启动快速拉取：全量拉取 BaoStock 历史分钟线（2019-01-02 至今）
#
# ★ V4.6 修复：原来 30/60/15分钟只拉最近 N 天（如 180天≈640根），
#   导致线段中枢跑不出来（需要约 2000+ 根），大量股票显示"中枢数据不足"。
#   BaoStock 分钟线从 2019-01-02 起，约 2650 日历天，直接全量拉取一次性解决。
#   日线/周线数据量不大，维持原有策略。
QUICK_FETCH_DAYS = {
    "week": 1825,  # 5 年周线 ≈ 260 根
    "day":  730,   # 2 年日线 ≈ 500 根
    "60":   2650,  # BaoStock 60m 全量 2019-至今 ≈ 13000 根（线段中枢充足）
    "30":   2650,  # BaoStock 30m 全量 2019-至今 ≈ 13000 根（线段中枢充足）
    "15":   2650,  # BaoStock 15m 全量 2019-至今 ≈ 26000 根（线段中枢充足）
    "5":    2650,  # BaoStock 5m  全量 2019-至今 ≈ 80000 根（线段中枢充足）
}
...
def fetch_klines_quick(
    ...
    count = upsert_klines(symbol, freq, rows, adjustflag, update_meta=False)
